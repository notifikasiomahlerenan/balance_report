  ...
  const listener = onValue(monthRef, (snapshot) => {
    ...
    const list: Expense[] = Object.entries(data).map(([id, val]) => ({
      ...
      ...(() => {
        ...
        // Backward-compat: if v1 entries only have `amount`, treat it as a debit.
...
// ─── Receipt image: base64 stored directly in Firebase DB ────────
// expo-image-picker captures with base64: true at quality 0.3.
// No manipulation library needed — camera handles compression at capture time.
